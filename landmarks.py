...
def select_landmarks(
    ...
    if method == SelectLandmarksMethod.RANDOM:
        ...
        sorted_all_vertices = sorted(graph.get_all_vertices(), key=lambda v: len(graph.neighbors[v]), reverse=True)
...
class LandmarksBasic:
    def __init__(
        ...
        self._landmarks = select_landmarks(graph, count_landmarks, select_landmarks_method)
        self._distances = {}
        self._prepare(graph)
    ...
    def distance(self, start: int, end: int) -> int:
        ...
        for landmark in self._landmarks:
            ...
            upper_bounder = min(upper_bounder, landmark_distance[start] + landmark_distance[end])
